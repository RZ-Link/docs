---
outline: deep
---

# 虚拟化
## 安装虚拟化组件
安装qemu组件
```shell
yum install -y qemu
```
安装libvirt组件
```shell
yum install -y libvirt
```
解决权限问题
```shell
vi /etc/libvirt/qemu.conf

#user = "qemu"
#group = "qemu"
user = "root"
group = "root"
```
启动libvirtd服务
```shell
systemctl start libvirtd
```
查看内核是否支持KVM虚拟化，查看/dev/kvm和/sys/module/kvm文件是否存在。文件存在，说明内核支持KVM虚拟化
```shell
ls /dev/kvm
ls /sys/module/kvm
```
安装bridge-utils软件包
```shell
yum install -y bridge-utils
```
安装virt-install软件包
```shell
yum install -y virt-install
```

## VXLAN
### 相同服务器的虚拟机互相访问
服务器1创建网桥br0
```shell
brctl addbr br0
ip link set dev br0 up
```
服务器1启动虚拟机1，设置静态IP为10.0.0.10，子网掩码为255.255.255.0
```shell
virt-install --virt-type kvm --name cirros1 --ram 512 --disk cirros1.img,format=qcow2 --network bridge=br0,model=virtio --console pty,target_type=serial --noautoconsole --os-type=linux --os-variant=unknown --import
```
服务器1启动虚拟机2，设置静态IP为10.0.0.20，子网掩码为255.255.255.0
```shell
virt-install --virt-type kvm --name cirros2 --ram 512 --disk cirros2.img,format=qcow2 --network bridge=br0,model=virtio --console pty,target_type=serial --noautoconsole --os-type=linux --os-variant=unknown --import
```
虚拟机1访问虚拟机2
```shell
ping 10.0.0.20
```
虚拟机2访问虚拟机1
```shell
ping 10.0.0.10
```

### 不同服务器的虚拟机互相访问
服务器2创建网桥br0
```shell
brctl addbr br0
ip link set dev br0 up
```
服务器2启动虚拟机3，设置静态IP为10.0.0.30，子网掩码为255.255.255.0
```shell
virt-install --virt-type kvm --name cirros3 --ram 512 --disk cirros3.img,format=qcow2 --network bridge=br0,model=virtio --console pty,target_type=serial --noautoconsole --os-type=linux --os-variant=unknown --import
```
服务器1创建vxlan0，将vxlan0添加到br0
```shell
ip link add vxlan0 type vxlan id 42 group 239.1.1.1 dev ens33 dstport 4789
ip link set dev vxlan0 up
ip link set vxlan0 master br0
```
服务器2创建vxlan0，将vxlan0添加到br0
```shell
ip link add vxlan0 type vxlan id 42 group 239.1.1.1 dev ens33 dstport 4789
ip link set dev vxlan0 up
ip link set vxlan0 master br0
```
虚拟机1访问虚拟机3
```shell
ping 10.0.0.30
```
虚拟机3访问虚拟机1
```shell
ping 10.0.0.10
```

## 搭建网桥，连接物理网络
### 搭建Linux网桥，连接物理网络
安装bridge-utils软件包
```shell
yum install -y bridge-utils
```
创建网桥br0
```shell
brctl addbr br0
ip link set dev br0 up
```
br0添加物理网卡ens34
```shell
brctl addif br0 ens34
```
启动虚拟机
```shell
virt-install --virt-type kvm --name cirros1 --ram 512 --disk cirros1.img,format=qcow2 --network bridge=br0,model=virtio --console pty,target_type=serial --noautoconsole --os-type=linux --os-variant=unknown --import
```

### 搭建Open vSwitch网桥，连接物理网络
安装Open vSwitch组件
```shell
yum install -y openvswitch
```
启动Open vSwitch服务
```shell
systemctl start openvswitch
```
创建Open vSwitch网桥br1
```shell
ovs-vsctl add-br br1
```
br1添加物理网卡ens34
```shell
ovs-vsctl add-port br1 ens34
```
启动虚拟机
```shell
virt-install --virt-type kvm --name cirros1 --ram 512 --disk cirros1.img,format=qcow2 --network bridge=br1,model=virtio,virtualport_type=openvswitch --console pty,target_type=serial --noautoconsole --os-type=linux --os-variant=unknown --import
```

## 路由

创建网桥br0
```shell
brctl addbr br0
ip link set dev br0 up
```
启动虚拟机1，设置静态IP为10.0.0.10，子网掩码为255.255.255.0
```shell
virt-install --virt-type kvm --name cirros1 --ram 512 --disk cirros1.img,format=qcow2 --network bridge=br0,model=virtio --console pty,target_type=serial --noautoconsole --os-type=linux --os-variant=unknown --import
```
创建网桥br1
```shell
brctl addbr br1
ip link set dev br1 up
```
启动虚拟机2，设置静态IP为20.0.0.10，子网掩码为255.255.255.0
```shell
virt-install --virt-type kvm --name cirros2 --ram 512 --disk cirros2.img,format=qcow2 --network bridge=br1,model=virtio --console pty,target_type=serial --noautoconsole --os-type=linux --os-variant=unknown --import
```
创建路由器（网络命名空间）
```shell
ip netns add router1
```
创建网线，连接router1和br0
```shell
# 创建网线
ip link add veth1-router1 type veth peer name veth1-br0
# 连接router1
ip link set veth1-router1 netns router1
ip netns exec router1 ip link set veth1-router1 up
# 连接br0
brctl addif br0 veth1-br0
ip link set veth1-br0 up
# 设置路由器端口IP
ip netns exec router1 ip addr add 10.0.0.1/24 dev veth1-router1
```
创建网线，连接router1和br1
```shell
# 创建网线
ip link add veth2-router1 type veth peer name veth2-br1
# 连接router1
ip link set veth2-router1 netns router1
ip netns exec router1 ip link set veth2-router1 up
# 连接br1
brctl addif br1 veth2-br1
ip link set veth2-br1 up
# 设置路由器端口IP
ip netns exec router1 ip addr add 20.0.0.1/24 dev veth2-router1
```
虚拟机添加默认路由
```shell
# 虚拟机1添加默认路由
ip route add default via 10.0.0.1
# 虚拟机2添加默认路由
ip route add default via 20.0.0.1
```
虚拟机1访问虚拟机2
```shell
ping 20.0.0.10
```


## 命令参考
查询主机上已经定义的所有虚拟机信息列表
```shell
virsh list --all
```
启动虚拟机
```shell
# vnc
virt-install --virt-type kvm --name openEuler1 --ram 2048 --disk openEuler-24.03-LTS-x86_64.qcow2,format=qcow2 --network bridge=br0,model=virtio --graphics vnc,listen=0.0.0.0 --noautoconsole --os-type=linux --os-variant=unknown --import
# console
virt-install --virt-type kvm --name cirros1 --ram 512 --disk cirros-0.6.3-x86_64-disk.img,format=qcow2 --network bridge=br0,model=virtio --console pty,target_type=serial --noautoconsole --os-type=linux --os-variant=unknown --import
```
启动虚拟机
```shell
virsh start <VMInstance>
```
关闭虚拟机
```shell
virsh shutdown <VMInstance>
```
销毁虚拟机
```shell
virsh undefine <VMInstance>
```
查询虚拟机使用的VNC端口号
```shell
virsh vncdisplay <VMInstance>
```
连接虚拟机串口
```shell
virsh console <VMInstance>
```

## Create images manually
Before starting a virtual machine with libvirt, verify that the libvirt default network has started.
```shell
virsh net-list
```
The following example shows how to use the qemu-img command to create an empty image file, and virt-install command to start up a virtual machine using that image file.
```shell
qemu-img create -f qcow2 openEuler2.qcow2 10G
virt-install --virt-type kvm --name openEuler2 --ram 2048 --disk openEuler2.qcow2,format=qcow2 --network network=default --graphics vnc,listen=0.0.0.0 --noautoconsole --os-type=linux --os-variant=unknown --location=openEuler-24.03-LTS-x86_64-dvd.iso
```
